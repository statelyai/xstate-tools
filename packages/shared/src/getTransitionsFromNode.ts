import { StateNode, StateValue } from "xstate";
...
export const getStateValues = (machine: StateNode) => {
  const allStateNodes = machine.stateIds.map((id) =>
    machine.getStateNodeById(id)
  );

  const states = allStateNodes.reduce((arr: StateValue[], node) => {
    return Object.keys(pathToStateValue(node.path)).length === 0
      ? arr
      : [...arr, pathToStateValue(node.path)];
  }, []);
