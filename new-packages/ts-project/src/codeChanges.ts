...
import { assert, findLast, findProperty, first, last } from './utils';
        ...
        if (!prop) {
          codeChanges.insertPropertyIntoObject(
            ...
            path
              ...
              .reverse()
              .reduce(
                (v, segment) => c.object([c.property(segment, v)]),
                value,
              ),
          );
          return;
  ...
  switch (element.type) {
    case 'array': {
      return (
        `[` +
        element.elements
          .map((e) => insertionToText(ts, sourceFile, e, formattingOptions))
          .join(', ') +
        `]`
      );
    }
